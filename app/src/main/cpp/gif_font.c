...
void
drawFrame(GifFileType* gif, AndroidBitmapInfo*  info, int* pixels, int frame_no, bool force_dispose_1)
{
    GifColorType *bg;
    GifColorType *color;
    SavedImage * frame;
    ...
    frame = &(gif->SavedImages[frame_no]);//取出这一帧图片
    frameInfo = &(frame->ImageDesc);//取出描述信息
    if (frameInfo->ColorMap) {//如果这一帧有自己的画板就采用自己的 不然就采用整体的
    ...
    if (frameInfo->Interlace && false) {
        ...
        px = px + info->stride*frameInfo->Top;
        for (y=frameInfo->Top; y<frameInfo->Top+frameInfo->Height; y++) {
            line = (int*) px;
            for (x=frameInfo->Left; x<frameInfo->Left+frameInfo->Width; x++) {
                loc = (y - frameInfo->Top)*frameInfo->Width + (x - frameInfo->Left);
